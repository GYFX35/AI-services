...
@app.route('/api/v1/digital-ecosystem/assistance', methods=['POST'])
@require_api_key
def digital_ecosystem_assistance_endpoint():
    data = request.get_json()
    prompt = data.get('prompt')
    if not prompt:
        return jsonify({"error": _("Prompt is required")}), 400
    message = google_ai.provide_digital_ecosystem_assistance(prompt)
    return jsonify({"status": "success", "message": message})
