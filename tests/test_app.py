...
@patch('google_ai.provide_eshop_assistance')
def test_eshop_assistance(mock_gen, client, auth_headers):
    mock_gen.return_value = 'Mock e-shop response'
    response = client.post('/api/v1/eshop/assistance',
                           data=json.dumps({'prompt': 'test e-shop'}),
                           content_type='application/json',
                           headers=auth_headers)
    assert response.status_code == 200
    data = json.loads(response.data)
